use strict;
my $pvalue_cutoff = 1E-9;  			# pValue cut-off for kinase - non-kinase gene pair.  
my $qvalue_cutoff = 1E-5;  			# qvalue cut-off for a kinase to be considered as predictor kinase for a module.
my $total_gene = 30172; 			# the total number of non-kinase genes.
...
my $edge_file = "./data/At.SigEdges.txt"; 		# with singinificant interacting kinase-target gene pair.
my $output_file = "results.regulator.kinases.txt";
...
my %e;		# to store predictor kinases info for target genes.
my %tfc;	# number of target genes for a kinase within the genome.
my %seen;
my $l;
my @t;
my %symbl;
...
unless( -e $edge_file ){
	print "The file 'At.SigEdges.txt' not found. It should be saved in the 'data' sub-directory. And the location of the script 'getArabidopsisRegulatorKinases.pl' should not be changed.\nExit in 5 second.\n";
	sleep 5;
	exit 1;
}

unless( -e $gene_symbl_file ){
	print "The file 'At.gene.symbls.txt' not found. It should be saved in the 'data' sub-directory. And the location of the script 'getArabidopsisRegulatorKinases.pl' should not be changed.\nExit in 5 seconds.\n";
	sleep 5;
	exit 1;
}

unless( -e $target_gene_file ){
	print "Target gene file 'At.target.genes.txt' not found. It should be saved in the 'data' sub-directory. And the location of the script 'getArabidopsisRegulatorKinases.pl' should not be changed.\nExit in 5 seconds.\n";
	sleep 5;
	exit 1;
}

unless( -e $cluster_file ){
	print "The file 'modules_to_analyze.txt' not found. It should be saved in the same directory as the script 'getArabidopsisRegulatorKinases.pl'.\nExit in 5 seconds.\n";
	sleep 5;
	exit 1;
}
...
print OUT "Module\tRank\tKinase\tSymbl\tModuleSize(without kinase genes)\tCount\tCountInGenome\tGenomeSize\tpValueEnrichment\tpValue(bh-adjusted)\tFraction\tmean_beta\tTargetAGI\tTargetSymbl\tbeta\tTargetpValue( -log10 p )\n";
foreach $k ( @c ){
	$l = $g{$k};
	$l =~ s/\t$//;
	@t = split("\t",$l);

...
getChordDiagram <- function ( module, kinasenum = 50, targetnum = 15, ratio = 1, cex = 1)
{
	if (!file.exists("results.regulator.kinases.txt")){
		cat ("The file 'results.regulator.kinases.txt' not found.\n")
		return("Exit")
	}

	if (!file.exists("getChordLists.pl")){
		cat ("The file 'getChordLists.pl' not found. This file is required for extracting the kinase-non-kinase target gene pairs for the input module. It should be placed within the home folder of the EXPLICIT-Kinase package.\n")
		return("Exit")
	}
	a <- read.table("results.regulator.kinases.txt",heade=T,sep="\t")
	if ( module %in% a[,1]){
		unlink( "chord.lists.txt" ) 
		perlcmd = paste("perl getChordLists.pl",module,kinasenum,targetnum,sep=" ")
		system( perlcmd )
		...
		cat ("The module ",module," was not found in 'results.regulator.kinases.txt'.","\n",sep="")
	}
}
